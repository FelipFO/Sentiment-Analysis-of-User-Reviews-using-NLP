...
def get_performance(
    ...
    precision = round(metrics.precision_score(y_test, predictions), 5)
    recall = metrics.recall_score(y_test, predictions)
    f1_score = round(metrics.f1_score(y_test, predictions), 5)
    report = metrics.classification_report(y_test, predictions, target_names=[str(x) for x in labels])
